...
class Dense3DSpatialTransformer(Layer):
    """Dense 3D spatial transformer with trilinear interpolation."""
    ...
    def build(self, input_shape):
        if len(input_shape) > 3:
            raise ValueError(
                "Spatial Transformer must be called on a list of length 2 or 3. "
                "First argument is the image, second is the offset field."
            )

        if len(input_shape[1]) != 5 or input_shape[1][4] != 3:
            raise ValueError(
                "Offset field must be one 5D tensor with 3 channels. "
                f"Got: {input_shape[1]}"
            )
    ...
    def _transform(self, image, dx, dy, dz):
        ...
        x_mesh = tf.tile(tf.expand_dims(x_mesh, 0), [batch_size, 1, 1, 1])
        y_mesh = tf.tile(tf.expand_dims(y_mesh, 0), [batch_size, 1, 1, 1])
        z_mesh = tf.tile(tf.expand_dims(z_mesh, 0), [batch_size, 1, 1, 1])

        return self._interpolate(image, dx + x_mesh, dy + y_mesh, dz + z_mesh)
    ...
    def _meshgrid(self, height, width, depth):
        x_t = tf.matmul(
            ...
            tf.transpose(
                tf.expand_dims(
                    tf.linspace(0.0, tf.cast(width, tf.float32) - 1.0, width), 1
                ),
                [1, 0],
            ),
        )
        y_t = tf.matmul(
            tf.expand_dims(
                tf.linspace(0.0, tf.cast(height, tf.float32) - 1.0, height), 1
            ),
    ...
    def _interpolate(self, im, x, y, z):
        ...
        x = tf.cast(tf.reshape(x, [-1]), "float32") + 1
        y = tf.cast(tf.reshape(y, [-1]), "float32") + 1
        z = tf.cast(tf.reshape(z, [-1]), "float32") + 1
        ...
        z0 = tf.clip_by_value(tf.cast(tf.floor(z), "int32"), 0, max_z)
        z1 = tf.clip_by_value(z0 + 1, 0, max_z)
        ...
        return tf.reshape(output, tf.stack([-1, out_height, out_width, out_depth, channels]))
